...
import { setTemplateData } from "../templateService.js";
...
async function getTemplates(userSession){
    ...
    const response = await axios.get("https://graph.facebook.com/v22.0/"+waba_id+"/message_templates", {headers: {Authorization: "Bearer "+userSession.accessToken}})
...
async function sendTemplates(message, userSession) {
    ...
    if(messageType == "document"){
        ...
        const template_response = await axios.get(
          `https://graph.facebook.com/v22.0/${waba_id}/message_templates?name=${userSession?.templateName}`,
